	...
	<tbody>
		<tr>
			<th style="border-bottom: 2px solid #ddd; border-top: 2px solid #ddd; ">
				<!-- title -->
				<input style="height: 25px;" type="text" value="[뉴스] ${boardVO.title }" name="title" readonly/>
			</th>	
		</tr>
		<tr>
			<td>
...
<%-- 	<script src="<c:url value='../resources/assets/js/reply.js'/>" /></script>	 --%>
				...
				$.getJSON(url, function(data){
					...
					$(data).each(function(){
						...
						/* list += '<li class ="reply_list" data-rno="'
							...
							+dateString+' '
							+'</span>'						
							+ '<button class="selecter" id="'+num+'">수정</button>'								
							+'</li>'; */
						list += '<tr class ="reply_list" data-rno="'+this.rno+'" id="replies" name="commentParentCode">'+
						'<th width="60px" height="60px"><img src="'+this.profileimage+'" width="60px" height="60px"/></th>'+
                        '<td colspan=2>'+
