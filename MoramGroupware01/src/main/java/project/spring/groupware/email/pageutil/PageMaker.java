...
public class PageMaker{
	...
	//
	private NewPaginationCriteria criteria2;
	...
	public NewPaginationCriteria getCriteria2() {
		return criteria2;
	}
	...
	public void setCriteria2(NewPaginationCriteria criteria2) {
...
//		
	...
	public void setNewPageDate(){
		//1페이지라면, 1페이지/5개씩보여주기로햇으니 
		//..(3일때 5,) (4일때, 5).. 데이터는 고려되진 않음. 
		int maxPageLink=(int)Math.ceil((double)totalCount/criteria2.getPerPage());				
//		curBlock=(int)Math.ceil((curBlock-1)/numOfPageLink+1);
//		startPageNum=(curBlock-1)*numOfPageLink-1;
		//end 는 5, 10, 15 , 20 .. 
		int temp=(int)(Math.ceil(criteria2.getPage()/(double)numOfPageLink)
				*numOfPageLink);
...
//		
		...
		if(endPageNum*criteria2.getPerPage()<totalCount){
			hasNext=true;
		}else{
			hasNext=false;
		}
